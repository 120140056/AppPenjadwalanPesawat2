...
import { editItem } from './actions/editAction';
import { deleteItem } from './actions/deleteAction';

// data dummy
const rows = useSelector(state => state.items);
    ...
    <View style={styles.item}>
      ...
      <View style={styles.item4}>
        ...
        <TouchableOpacity onPress={() => {
          ...
          <Image source={require('./assets/n-delete.png')} style={{ width: 20, height: 20 }}/>
        </TouchableOpacity>
      </View>
    </View>
  );
...
function Form({ route, navigation }) {
  ...
  const handleAddItem = () => {
    const newItem = { asal: origin, tujuan: destination, tanggal: departureDate, merk: plane };
